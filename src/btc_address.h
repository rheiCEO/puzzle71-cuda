...
__global__ void __launch_bounds__(BLOCK_SIZE, 2) gpu_address_work(int score_method, CurvePoint* offsets) {
    ...
    handle_output(score_method, calculate_hash160_compressed(p.x, p.y), key, 0);
    ...
    for (int i = THREAD_WORK - 2; i >= 1; i--) {
        ...
        handle_output(score_method, calculate_hash160_compressed(curve_x, curve_y), key + i + 1, 0);
    }
    ...
    handle_output(score_method, calculate_hash160_compressed(curve_x, curve_y), key + 1, 0);
}
